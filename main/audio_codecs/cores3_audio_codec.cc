...
void CoreS3AudioCodec::SetOutputVolume(int volume) {  // 设置输出音量
    ESP_ERROR_CHECK(esp_codec_dev_set_out_vol(output_dev_, volume));
    AudioCodec::SetOutputVolume(volume);
}

void CoreS3AudioCodec::EnableInput(bool enable) {  // 启用输入
    ...
    if (enable) {
        ...
        if (input_reference_) {
            fs.channel_mask |= ESP_CODEC_DEV_MAKE_CHANNEL_MASK(1);  // 通道掩码
        }
        ESP_ERROR_CHECK(esp_codec_dev_open(input_dev_, &fs));  // 打开输入设备
        ESP_ERROR_CHECK(esp_codec_dev_set_in_channel_gain(input_dev_, ESP_CODEC_DEV_MAKE_CHANNEL_MASK(0), 40.0));  // 设置输入通道增益
    } else {
        ESP_ERROR_CHECK(esp_codec_dev_close(input_dev_));
    }
    AudioCodec::EnableInput(enable);  // 启用输入
}

void CoreS3AudioCodec::EnableOutput(bool enable) {  // 启用输出
